...
This document defines the semantic meaning and intended usage of the OVDS v0.2.0 schemas.

The JSON Schema files define structure. This document defines normative meaning.

## Entity Model

OVDS v0.2.0 includes the following primary entities:
...
Fuel-cell stacks are not yet defined as a standalone reusable schema in v0.2.0.
...
The v0.2.0 identifier shape is:
...
The `OVDS` namespace prefix is used consistently in v0.2.0 to match the standard name and avoid ambiguity with external identifier systems.
...
- `vehicle.id` is the most granular published identifier in v0.2.0 and should be unique for each concrete configuration record.
...
The `compliance.approvals` block is the primary place for official homologation, certification, type-approval, or conformance records issued by a regulatory authority or recognized scheme owner.

Each approval record is jurisdiction-agnostic and may describe an EU type approval, an EPA certification, a Mexico NOM certification, or another market-specific approval without changing the surrounding vehicle model.

Approval records should reuse the vehicle lineage already present on the enclosing vehicle record. Fields such as `modelYear` should not be duplicated inside the approval entry unless a future version defines a specific reason to do so.

`sourceDocs` allows one or more official supporting documents to be attached to an approval record, such as certificates, authority publications, annexes, regulations, or public registry entries.
...
When the source is more specific, `emissionsControls` may also describe catalyst-system type and placement, plus emissions-sensor type and placement. This makes it possible to distinguish, for example, a close-coupled three-way catalyst from an underfloor SCR stage, or an upstream wideband oxygen sensor from a downstream monitoring sensor.
...
- `compliance.approvals` contains structured approval or certification records tied to a jurisdiction, authority, scheme, and supporting source documents.

Each `compliance.approvals` entry may identify:

- the issuing authority
- the applicable jurisdiction
- the approval or certification scheme
- the approval domain, such as emissions, safety, or market access
- the applicable standard or regulation when known
- an approval identifier and lifecycle status
- supporting `sourceDocs`

Jurisdiction-specific fields that do not generalize cleanly across markets should be carried in the `references` object so the core structure remains portable.

## Safety Semantics

Safety features are modeled in a dedicated schema and grouped by capability area rather than by commercial package.

The v0.2.0 safety model uses grouped boolean features for cross-brand comparability.

This taxonomy is intentionally simple in v0.2.0 and may evolve in future versions without changing the role of safety as a standalone comparable feature set.
...
This `readme.md` file is the normative source for v0.2.0 semantic rules.
